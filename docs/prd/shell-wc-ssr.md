...
- ✅ **End-to-end verified** on Angular `/overlays/shell`, the full no-JS matrix via the Phase-0 spikes (Chromium), and React + Vue SSR (HTML output + live Chromium: top-bar hamburger, wide push, narrow full-width overlay drawer, 0 errors).

**Remaining:** Phase 6 (Docker/Traefik + React/Vue/Angular **prod** build targets: split `browser/` + `vite build --ssr` server bundle, `compression`, NODE_ENV=production serve config), Phase 7 (tests + demo pages). Smaller follow-ups: external-trigger **global** bridge stylesheet (+ outer-vs-`:host` cascade check), a **codegen Nx target** to own the chrome regen (currently a manual script), and **Firefox** parity. Phase-0/1 spike scratch files live under `docs/prd/_spike-*` (throwaway, not committed).
...
- **React** (`libs/mintplayer-react-bootstrap/shell/` + demo): **Preferred for the shell — reuse `injectMpShellDsd`.** Because `mp-shell`'s DSD chrome is *static*, React SSR can render to HTML (`renderToString`/`renderToPipeableStream`) and then apply the **same framework-agnostic `injectMpShellDsd(html)` post-render helper** used by Angular — no `@lit-labs/ssr-react` required for the shell. (`@lit-labs/ssr-react`'s `enable-lit-ssr.js` `createElement` monkey-patch is only needed for WCs with *dynamic* shadow content; keep it in mind for those, not the shell.) **Prereq:** relocate `injectMpShellDsd` + `MP_SHELL_DSD_CHROME` from `@mintplayer/ng-bootstrap/shell` (Angular) to `@mintplayer/web-components/shell` (framework-agnostic), with the Angular lib re-exporting for back-compat, so React/Vue servers import them without an Angular dependency. The existing `@lit/react` `createComponent` wrapper needs no layout code.
...
- **Phase 4 — React SSR:** **(0)** relocate `injectMpShellDsd`/`MP_SHELL_DSD_CHROME` → `@mintplayer/web-components/shell` (framework-agnostic; Angular re-exports; `gen-shell-chrome` output path moves). **(1)** entry-client/entry-server/server (Vite SSR + Express + `StaticRouter`), inject DSD via the shared helper (no `@lit-labs/ssr-react` for the static-chrome shell). **(2)** React `BsShell` wrapper (`@lit/react` `createComponent`; sidebar = child with `slot="sidebar"`). **(3)** replace `AppShell.tsx`'s outer layout with `BsShell`.
