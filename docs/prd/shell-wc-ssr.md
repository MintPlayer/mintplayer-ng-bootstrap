...
**All phases (0–7) and follow-ups are complete.** The PR is open: **#381** (`feat/shell-wc-ssr` → `master`). Post-PR polish commits on the branch: height-consistency fix, Angular hydrate-support, demo gray frame, codegen target + Firefox, external-toggle decision.

## Post-merge polish + OPEN ISSUES (in progress, 2026-06-02 — branch `feat/shell-wc-ssr`, PR #381)

Two issues reported on the Angular `/overlays/shell` demo are **NOT yet resolved**. There is also **uncommitted WIP** (a partial, wrong attempt at issue A — see below).

### Issue A — toggle ↔ auto/show/hide radios don't form a coherent state machine ⬜ OPEN
- **Symptoms:** Wide mode — the hamburger collapses the sidebar, but clicking it *again does nothing* (can't reopen). Narrow mode — "Auto" is initially marked, but clicking the hamburger *does nothing*.
- **Root cause:** the demo's `onShellToggle` maps `statechange.detail.open` (which is the **raw checkbox** `.checked`) → `state='show'|'hide'`. But once `state` is `show`/`hide`, the WC's `:host([state=show])/([state=hide])` CSS **overrides** the internal checkbox (`--mp-shell-open` forced), so flipping the checkbox again has no visual effect → "clicking again does nothing." Also `statechange.open` (raw checkbox) is the **inverse** of the visual state in `auto`+wide, so the `open→show/hide` mapping is semantically wrong. Verified live: at wide+auto, one hamburger click emits `statechange{open:true}` yet the demo ends at `state=hide` (the mapping + override interact incorrectly).
- **Intended fix (per user's reference):** model a small state machine like MintPlayer's `MintPlayer.Web/ClientApp/src/app/app.component.ts` (~lines 140–225): a `toggleButtonState` (auto/open/closed) + `sidebarState` (auto/show/hide), where the hamburger click computes the **next** state from the current state **and** the viewport width (768 breakpoint), `updateSidebarState()` maps toggleButtonState→sidebarState, and `collapseSidebar()` forces `hide` when `window.innerWidth < 768`. The radios reflect `sidebarState`.
- **Design implication to decide:** the WC's `statechange`/`open` currently report the **raw checkbox**, not the **visual** open state. For a consumer state machine to be sane, either (a) change `mp-shell` so `open`/`statechange` report the resolved visual open state, or (b) have the demo's hamburger NOT use the built-in checkbox toggle and instead drive `[state]` directly (treat the hamburger as an external control computing the next state). Lean toward making the demo own the state machine and possibly using `external-toggle` + a consumer-driven toggle, OR fixing `statechange` semantics. **Revisit before implementing.**
- **Files:** `apps/ng-bootstrap-demo/src/app/pages/overlay/shell/shell.component.{ts,html}`; possibly `libs/mintplayer-web-components/shell/src/components/mp-shell.ts` (statechange/open semantics) + `shell.styles.scss`; `libs/mintplayer-ng-bootstrap/shell/src/shell/shell.component.{ts,html}` (the `statechange` output added below).

### Issue B — transient duplicate toggle-button still flashes on Angular reload ⬜ OPEN
- **Symptom:** on reload of `/overlays/shell`, a **second sidebar toggle-button briefly appears below the sidebar**, then disappears. Persists even after the eager-import + hydrate-support fix **and** a dev-server restart.
- **Steady state is clean** (verified: 1 `mp-shell`, 1 `bs-shell`, single shadow chrome, 208px, 0 console errors) — the duplicate is **transient/sub-frame**, so a post-load e2e can't catch it (Angular's destructive recreate always settles on a clean single shadow).
- **Already tried (committed):** eager client-side WC registration in `apps/ng-bootstrap-demo/src/main.ts` (so it upgrades synchronously) + `import '@lit-labs/ssr-client/lit-element-hydrate-support.js'` *before* the WC import (so the SSR DSD hydrates instead of duplicating, like React/Vue). Did **not** fully fix it.
- **Likely remaining cause / next step:** Angular's **destructive (non-hydrating) bootstrap** recreates `<mp-shell>` via `createElement`; the SSR element's early upgrade may still duplicate before Angular destroys it, or Lit's async (microtask) first render on the recreated element paints a duplicate/stacked frame. **Instrument with Playwright `addInitScript` + a `MutationObserver`** (via `browser_run_code_unsafe`) to catch the transient and identify which element/render produces the second toggle. Then consider whether `provideClientHydration()` (avoiding the destructive recreate entirely — note the e2e cost, see [[project_e2e_destructive_bootstrap]]) is the real fix.
- **Files:** `apps/ng-bootstrap-demo/src/main.ts`, `apps/ng-bootstrap-demo/src/app/app.config.ts`, `apps/ng-bootstrap-demo/server.ts`, `libs/mintplayer-ng-bootstrap/shell/src/shell/shell.component.ts` (`afterNextRender`), `libs/mintplayer-web-components/shell/src/components/mp-shell.ts`.

### UNCOMMITTED WIP (partial fix for Issue A — being reworked, do not assume correct)
- `libs/mintplayer-ng-bootstrap/shell/src/shell/shell.component.ts` + `.html`: added a `statechange = output<ShellStateChangeEventDetail>()` to `bs-shell` re-emitting the WC's `statechange` (this part is reusable/good).
- `apps/ng-bootstrap-demo/src/app/pages/overlay/shell/shell.component.{ts,html}`: added `(statechange)="onShellToggle($event)"` + `onShellToggle(detail) { this.shellState = detail.open ? 'show' : 'hide'; }` — **this mapping is the buggy part** (causes Issue A). Replace with the proper state machine.
