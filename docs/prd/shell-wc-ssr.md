...
**All phases (0–7) and follow-ups are complete.** The PR is open: **#381** (`feat/shell-wc-ssr` → `master`). Post-PR polish commits on the branch: height-consistency fix, demo gray frame, codegen target + Firefox, external-toggle decision. **Final SSR/hydration fix (Issue A + B below):** the Angular demo now uses live `RenderMode.Server` + `provideClientHydration()` (it previously prerendered, which dropped the hydration transfer-state → NG0505 → destructive duplicate), and the WC handles its SSR Declarative Shadow DOM in `createRenderRoot` (the earlier app-level Angular hydrate-support shim was removed — it patched the wrong `lit` instance). React/Vue hydration (line above) is unchanged.

## Post-merge polish + follow-up fixes (2026-06-02 — branch `feat/shell-wc-ssr`, PR #381) — both issues ✅ RESOLVED

### Issue A — toggle ↔ auto/show/hide radios don't form a coherent state machine ✅ RESOLVED (2026-06-02)
- **Symptoms (reported):** Wide mode — the hamburger collapsed the sidebar, but clicking it *again did nothing* (couldn't reopen). Narrow mode — "Auto" was marked, the hamburger toggled the sidebar but the radios didn't light up to match.
- **Two root causes, both fixed:**
  1. **WC reported the wrong open state.** `mp-shell.open` / `statechange.detail.open` returned the **raw checkbox** `.checked`, which in `auto`+wide is the *inverse* of the visual state (unchecked = open). And once `state` was `show`/`hide`, the `:host([state])` CSS froze `--mp-shell-open`, so flipping the checkbox had no visual effect → "clicking again does nothing." **Fix:** `mp-shell` now reports the **resolved visual** open state, read straight off the value the CSS computed (`getComputedStyle('.sidebar-root').--mp-shell-open`) — the CSS matrix stays the single source of truth, no breakpoint logic duplicated in JS. `#onToggleChange` emits the resolved value in `auto`, or the **flip** of the frozen value when `state` is explicit (so each click keeps alternating). `#syncCheckbox` keeps the checkbox encoding the resolved open for the current viewport (wide vs narrow detected via `.sidebar` `position`), so returning to `auto` doesn't jump.
  2. **Angular double-fired the event.** The WC's `statechange` is `bubbles + composed`; it bubbled from `mp-shell` to the consumer's `<bs-shell>` host, where Angular's `(statechange)` binding caught it a **second time** with the raw `CustomEvent` (Angular doesn't unwrap `.detail`, so `.open` was `undefined` → always mapped to `'hide'`), clobbering the correct typed-output value. **Fix:** the Angular wrapper's `onStatechange` calls `event.stopPropagation()` and re-publishes via its typed `output()` — the Angular-facing API. (React/Vue add a single direct listener, so they were never affected.)
- **Demo:** `onShellToggle(detail) { this.shellState = detail.open ? 'show' : 'hide'; }` — correct now that `detail.open` is the resolved visual state; `[state]="shellState"` drives the WC (controlled), the WC never mutates its own `state`.
- **Verified:** Playwright, both viewports — wide `auto→hide→show→hide…`, narrow `auto→show→hide…`, radios reflect state, radios also drive the shell (reverse direction), `auto` re-entry doesn't jump. User confirmed both modes manually.
- **e2e (regression guard):** `apps/ng-bootstrap-demo-e2e/e2e/shell-toggle.spec.ts` (3 JS-enabled specs, green on Chromium + Firefox) — complements the existing no-JS `shell-nojs.spec.ts`. Uses a deterministic "WC upgraded" wait (not `networkidle`, which hangs on Firefox against the dev server's HMR socket).
- **Files:** `libs/mintplayer-web-components/shell/src/components/mp-shell.ts`; `libs/mintplayer-ng-bootstrap/shell/src/shell/shell.component.{ts,html}`; `apps/ng-bootstrap-demo/src/app/pages/overlay/shell/shell.component.{ts,html}`; `apps/ng-bootstrap-demo-e2e/e2e/shell-toggle.spec.ts`.

### Issue B — duplicate toggle-button flashes on Angular reload ✅ RESOLVED (2026-06-02)
- **Symptom (reported + confirmed via the user's screen recording):** on reload of `/overlays/shell`, a second sidebar toggle-button briefly appears **below the sidebar**, then disappears. The user extracted GIF frames: **frames 96–105 show TWO hamburgers**, frame 95 and 106 show one. So it IS a real, multi-frame duplicate (my earlier rAF sampler sampled between those frames and missed it — "could not reproduce" was wrong).
- **Two independent root causes, both fixed.** The fix that mattered for the user (live Node SSR + hydration) and a WC-internal one that the first fix unmasked:

  **(1) Angular wasn't hydrating — it was destructively re-rendering.** The demo had **no `provideClientHydration()`** and (worse) the app defaulted every route to **`RenderMode.Prerender` (SSG)**. Angular's prerender output ships the per-element `ngh` annotations but **NOT the `<script id="ng-state">` hydration transfer-state**, so the client logged **`NG0505`** ("no serialized information present in the server response") and fell back to a **destructive bootstrap**: it built a fresh component tree — a second `<mp-shell>` + its own shadow hamburger — and inserted it *before* discarding the SSR one. For ~10 frames both shells were live → "a second toggle below the sidebar." (Confirmed: with prerender, `ng-server-context="ssg"`, `ngh=161`, **no** `ng-state`, on both dev and the prod build, across all 99 prerendered routes.)
    - **Fix:** opt the app into **live Node SSR**. New `apps/ng-bootstrap-demo/src/app/app.routes.server.ts` (`{ path: '**', renderMode: RenderMode.Server }`) + `app.config.server.ts` (`provideServerRendering(withRoutes(serverRoutes))`, merged onto the shared browser `config`); `main.server.ts` now bootstraps with that server config. `app.config.ts` adds `provideClientHydration(withEventReplay())` and **drops `withEnabledBlockingInitialNavigation()`** (it contradicts hydration, NG05001).
    - **Result:** `ng-server-context` flips `ssg → ssr`, the `<script id="ng-state">` transfer-state is now emitted, `NG0505` is gone, and the client logs *"Angular hydrated 160 components … 0 skipped."* The build now reports **"Prerendered 0 static routes."**

  **(2) The WC appended a second copy inside its own shadow.** Once Angular *kept* the SSR `<mp-shell>` (with its parser-attached DSD shadow) instead of throwing it away, a new bug surfaced: on upgrade, lit-element's `createRenderRoot` pins `renderOptions.renderBefore` to the first SSR node in the shadow, so the first `render()` inserts a **second** copy of the chrome ahead of the inert DSD nodes (2 hamburgers *inside* the shadow). True client hydration of the DSD would need `@lit-labs/ssr-client`'s global patch to reach **this element's** `LitElement`, but **the app and the WC resolve different `lit` instances** (Vite pre-bundles the workspace lib's `lit` separately), so `globalThis.litElementHydrateSupport` patches the wrong class — verified: the registered `mp-shell` ran stock lit-element `createRenderRoot`/`update`, never the shim's.
    - **Fix (in the WC, bundler-independent):** `mp-shell.createRenderRoot()` clears the SSR Declarative-Shadow-DOM nodes (`this.shadowRoot?.replaceChildren()`) **before** `super.createRenderRoot()` captures them, then lets `render()` repopulate the shadow once. Styles are re-applied by `super` via `adoptedStyleSheets` (not children), so it's visually identical — the chrome is static, and the DSD still does its only job: the no-JS render before the element upgrades. This works for **every** consumer regardless of how the host app bundles `lit`, so the fragile app-level hydrate-support shim was **removed** (deleted `hydrate-support.ts`, reverted `main.ts` to a plain WC import + bootstrap).
- **Verified (Playwright, both dev `nx serve` SSR and the production `node server.mjs` build):** `maxShellsDuringLoad:1`, `maxHamburgersDuringLoad:1` across ~545 sampled frames (no transient duplicate), final shadow = 1 hamburger / 1 toggle / 1 sidebar-root + `adoptedStyleSheets:1`, **0 console errors, no NG0505**. Prod SSR HTML: `ng-server-context="ssr"`, `ng-state` present, `shadowrootmode`=1, `<mp-shell>`=1. All 750 WC unit tests + 8 shell e2e specs (chromium+firefox, `shell-toggle` + `shell-nojs`) green.
...
- `apps/ng-bootstrap-demo-e2e/e2e/shell-toggle.spec.ts` — NEW, 3 JS-enabled specs (green chromium+firefox).
- `apps/ng-bootstrap-demo/src/app/app.config.ts` — Issue B (`provideClientHydration(withEventReplay())`, removed `withEnabledBlockingInitialNavigation()`).
- `apps/ng-bootstrap-demo/src/app/app.routes.server.ts` — NEW, Issue B (`RenderMode.Server` for `**` → live Node SSR).
- `apps/ng-bootstrap-demo/src/app/app.config.server.ts` — NEW, Issue B (`provideServerRendering(withRoutes(serverRoutes))` merged onto `config`).
- `apps/ng-bootstrap-demo/src/main.server.ts` — Issue B (bootstrap with the server config).
- `apps/ng-bootstrap-demo/src/main.ts` — Issue B (reverted to plain WC import + bootstrap; hydrate-support shim removed).
...
- **Angular** (`bs-shell` wrapper, `libs/mintplayer-ng-bootstrap/shell/`):
  ...
  **Grounded design (confirmed against `apps/ng-bootstrap-demo/server.ts` + `app.config.ts`):**
  - `server.ts` runs `@angular/ssr`'s `AngularNodeAppEngine` (request-time SSR). *(Superseded 2026-06-02 — see Issue B above: the demo originally shipped with **no `provideClientHydration()`** + the default `RenderMode.Prerender`, i.e. a **destructive** client bootstrap, which caused the reload duplicate. It now uses **live `RenderMode.Server` + `provideClientHydration()`**, so Angular **reuses** the SSR DOM. The DSD-injection design below is unchanged and was always hydration-agnostic — see the next bullets.)*
...
- **Phase 7 — tests + docs:** 🟢 **DONE.** JS-disabled Playwright on all three (`shell-nojs.spec.ts`, all green); React/Vue e2e `webServer` switched static `preview` → SSR server; hydration regression guard (React/Vue `shell-hydration.spec.ts`); ARIA (`aria-label` toggle + `title` hamburger + `aria-label="Sidebar"` landmark). Demo pages: Angular `/overlays/shell` kept; React/Vue demo the shell *as* their app layout (no standalone page).
