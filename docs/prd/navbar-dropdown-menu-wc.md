...
## Round 8 (2026-07-22, branch `feat/navbar-wc-a11y`) — active-route highlighting restored

**Regression (user-reported):** since `17328c57` the active route is no longer highlighted (the item background used to become the primary color). Two-agent investigation:

**What legacy actually did (important correction):** there were ZERO custom `.active` rules — the highlight was 100% stock Bootstrap, reached through `::ng-deep` imports. Visuals by context: **top-level `.nav-link.active` = text recolor only** (`--bs-navbar-active-color`); **`.dropdown-item.active` = primary background + white text** (`--bs-dropdown-link-active-bg/color` — the "background became primary" memory is the menu items + submenu triggers). The class landed on the **anchor**: the demo's `NavbarRouterLinkActiveDirective` (`a[routerLink]`, still alive and unchanged) for leaf links, and the deleted `[bsNavbarTrigger]` (URL-prefix match, non-navigating) for dropdown triggers — including the **parent-trigger chain** of the active route.

**Root cause on the WC:** every styling hook keys off a host attribute (`:host([active])`, driven by an input nobody wires to the router) or the `.dropdown-item` class — never the anchor's class, which is where the router actually puts it. React/Vue were unaffected (they drive the `active` prop); the regression is Angular-only.

**Fix (framework-agnostic CSS first, thin Angular sugar on top):**
1. **Nav items:** `::slotted(a.active), ::slotted(button.active)` in `navbar-item.styles.scss` — the anchor IS the directly slotted child, so `::slotted` reaches it. Text recolor, stock-Bootstrap parity.
2. **Menu items:** the router-active class sits on the anchor NESTED inside the slotted `.dropdown-item` host — out of `::slotted`'s reach — so the rule goes in the **companion light-DOM sheet** (whose whole purpose is nested-anchor styling), in all three frameworks: `.dropdown-item > a.active { color: var(--bs-dropdown-link-active-color, #fff); background-color: var(--bs-dropdown-link-active-bg, var(--bs-primary)) }`. The anchor fills the padded row via the existing negative-margin reset, so this paints the FULL row (the legacy look). (`::slotted(.dropdown-item:has(> a.active))` is spec-valid and evergreen-supported but rejected — the companion sheet is the designated owner and needs no new selector machinery.) The existing `::slotted(.dropdown-item.active)` prop-driven path stays (React/Vue + `[active]` input).
3. **Dropdown triggers** (shadow-rendered — no light-DOM directive can ever class them): WC hooks `:host([active]) .nav-link.dropdown-toggle` (text recolor) and `:host([data-submenu][active])` (primary bg + white, the legacy submenu-trigger look), driven by an `active` host attribute. **Angular driving:** `BsNavbarDropdownComponent` gets `hostDirectives: [RouterLinkActive]` — RLA collects the projected descendant `RouterLink`s, its `isActiveChange` bridges to `[attr.active]` on the inner `<mp-navbar-dropdown>`. Non-exact matching ≈ the legacy prefix match but router-aware; nested dropdowns each report their own subtree, so the whole trigger chain highlights. Router is already a peer dep with library-side precedent (navigation-lock, scrollspy). React/Vue consumers set the same `active` attribute themselves.
4. **SSR/no-JS:** `routerLinkActive` classes and host bindings render server-side; all hooks are pure CSS → highlighting works with JS off (chrome regen required for the shadow rules).

Tokens: the active colors use the Bootstrap tokens (which inherit through the flat tree from the menu shadow), falling back to `var(--bs-primary)` — deliberately theme-tracking rather than legacy's compiled literal `#0d6efd`.
