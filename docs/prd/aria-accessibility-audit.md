...
- ~~**Drag-keyboard family**~~ ✓ **Shipped** for `scheduler`, `dock`, `tile-manager` via [`wc-aria-accessibility.md`](./wc-aria-accessibility.md) Phases 3/6/7 + the scheduler follow-up [`scheduler-keyboard-grid-nav.md`](./scheduler-keyboard-grid-nav.md). The library-wide trigger key settled on `M` for the dock + tile-manager (T/R/B/L/F commits for dock; arrow-nudge for tile-manager); scheduler later adopted `Enter` after the cell-level grid model shipped. ✓ **`mintplayer-ng-swiper`** also shipped 2026-05-11 via [`swiper-aria.md`](./swiper-aria.md) (`101a985b`): `bsSwipe` auto-emits per-slide `role="group"` + `aria-roledescription` + `aria-label="N of M"` + `aria-hidden` on offside clones; `bsSwipeContainer` owns ArrowLeft/Right/Up/Down + Home/End handling, `aria-orientation`, `aria-keyshortcuts`; `bsSwipeViewport` owns the live-region tuple (`aria-live` / `-atomic` / `-relevant` / `-busy`). `bs-carousel` migrated to the new contract — six pieces of duplicated wiring removed. **⚠️ Superseded by PR [#392](https://github.com/MintPlayer/mintplayer-ng-bootstrap/pull/392) (`207d85f7`): `@mintplayer/ng-swiper` and all three directives were deleted. The behaviour was preserved — and improved — inside the `<mp-carousel>` web component, with the framework-agnostic (ARIA-free) half extracted to `@mintplayer/web-components/swiper-core`. See [`swiper-aria.md`](./swiper-aria.md) §11 and [`carousel-wc.md`](./carousel-wc.md).**

### 13.2 Major follow-ups

- ✓ **Carousel play/pause button.** Shipped 2026-05-11. `<bs-carousel>` now renders a Play/Pause control whenever `[interval]` is set; consumers can override the default button via a `*bsCarouselPlayPause` projection slot (structural directive + `ngTemplateOutlet`, exposing `let-paused` / `let-toggle`). Two-way `[(paused)]="..."` model + public `play()` / `pause()` methods. `aria-live` is now computed: `off` while auto-advance is actually rotating, `polite` when paused / `interval` unset / `prefers-reduced-motion`. The audit's "auto-advance off by default" half was already satisfied — `[interval]` defaulted to `null` (no rotation) — so no BC needed there. **⚠️ Reshaped by PR #392, which rebuilt the carousel as `<mp-carousel>`: the `*bsCarouselPlayPause` structural directive is gone; a custom control is now projected with `slot="play-pause"` and wired to the element's `togglePaused()` / `pausedChange`. The built-in fallback button survives, and is rendered **first in DOM order** (`mp-carousel.ts:824-840`) so the stop-rotation control precedes the rotating content. The computed `aria-live` rule is unchanged but now lives in the element (`mp-carousel.ts:696-701`) and is emitted only in the browser tier. Rotation additionally pauses on hover and on focus-within (`mp-carousel.ts:654-682`, WCAG 2.2.2), and off-screen slides are `inert` rather than just `aria-hidden`. The two-way `[(paused)]` model and `play()` / `pause()` remain on `<bs-carousel>`.**
- ✓ **Virtual-datatable aria-rowcount / aria-rowindex.** Shipped 2026-05-11. `VirtualDatatableDataSource.length$` now exposes a `BehaviorSubject<number>` so the component can keep `aria-rowcount = totalRecords + 1` in sync without polling. Both the header `<table>` (via a new `[ariaRowCount]` input on `<bs-table>`) and the body `<table>` (raw `[attr.aria-rowcount]`) carry the same logical count. Header `<tr>` is `aria-rowindex="1"`; each `*cdkVirtualFor` row uses CDK's `let i = index` (absolute index in the source data, not render index) bound to `aria-rowindex="i + 2"`. Browser-verified end-to-end: `aria-rowcount="139"` on a 138-artist dataset, scrolling to the middle re-cycles rendered rows so they expose `aria-rowindex="59"…"77"` while the rowcount stays stable. **⚠️ Mechanism replaced (behaviour kept):** the separate `@mintplayer/ng-bootstrap/virtual-datatable` entrypoint, `BsVirtualDatatableComponent` and `VirtualDatatableDataSource.length$` no longer exist — virtual scrolling was absorbed into the `<mp-datatable>` web component, which computes `aria-rowcount` in `render()` and stamps `aria-rowindex` per row from the absolute source index (`mp-datatable.ts:816`, `:824`, `:947`). The `[ariaRowCount]` input on `<bs-table>` is still there (`table.component.ts:23`).
- ✓ **prefers-reduced-motion** on remaining components — fully shipped. Splitter / dock / scheduler in `332d2d4f`; marquee already had it; tile-manager from `ca7a0c9d`; carousel auto-advance with the APG bundle 2026-05-11; accordion slideUpDown in `86b4c319` (2026-05-11) along with a new `BsReducedMotionDirective` in `@mintplayer/ng-bootstrap/reduced-motion` that the carousel migrated to in the same commit. **⚠️ The directive no longer exists.** Its two consumers were rewritten as web components and both stopped needing it: the carousel reads a `MediaQueryList` directly (`mp-carousel.ts:129-132`, gating the autoplay timer at `:639-652` and the transition duration inside `IndexMachine`), and the accordion's animation became a `@media (prefers-reduced-motion: reduce)` rule in its shadow stylesheet — then moot entirely once the accordion moved to native `<details>`. With zero consumers left, `@mintplayer/ng-bootstrap/reduced-motion` was **deleted** (no deprecation shim). Every remaining reduced-motion guard in the workspace is a CSS media query, which is the right layer for it; a component needing the signal in TypeScript should read `window.matchMedia` itself. Parallax was reviewed and dropped — `background-attachment: fixed` is genuinely static (the image doesn't move on its own; scroll is user-controlled), so WCAG 2.3.3 doesn't apply.
- ✓ **More live-announcer consumers** (per §6.4) — fully shipped. ✓ tile-manager "Dragging X" / "Move blocked" (in Phase 3); ✓ scheduler "View changed", "Event added/updated/removed", "Move mode for X", "Move committed/cancelled", per-cell focus narration, selection range narration (Phases 5/6 + scheduler-keyboard-grid-nav PRD); ✓ dock pane move-mode entry/commit/cancel announcements (Phase 7); ✓ typeahead "N results found" / "1 result found" / `noSuggestionsText`; ✓ searchbox same triplet (gated on debounced in-flight requests so eager `[suggestions]` prepopulation doesn't fire); ✓ placeholder "Loading complete" on `isLoading` true→false transition. **⚠️ `bs-searchbox` was since deleted** (issue #342 replaced select2 / searchbox / multiselect with `<mp-tree-select>`); its result announcement carried over — `mp-tree-select.ts:384` announces `"1 result."` / `"N results."`, plus per-chip selected/removed/cleared announcements at `:643-645`.
...
- **Carousel APG bundle.** §13.2 row 1 closed. *(This entry records the May 2026 Angular implementation; PR #392 later moved all of it into `<mp-carousel>` — see the ⚠️ note on §13.2 row 1 for what the API looks like today.)* New `*bsCarouselPlayPause` structural directive — consumer projects a `<ng-template>` that receives `let-paused` and `let-toggle` from the carousel's context. Two-way `[(paused)] = model<boolean>(false)` plus public `play()` / `pause()` methods. When the consumer doesn't project a template but `[interval]` is set, the carousel falls back to a built-in default Play/Pause button (mirroring the `defaultFileTemplate` pattern in file-upload), so APG compliance is "correct by default" for any consumer who turns auto-advance on. `aria-live` is a computed signal — `off` while auto-advance is rotating, `polite` when paused / unconfigured / under prefers-reduced-motion. Reduced-motion suppresses the auto-advance timer entirely (no rotation; matches tile-manager / scheduler / dock semantics). The "auto-advance off by default" half was already satisfied since `[interval]` defaulted to `null`. Browser-verified: clicking the default button flips text "Pause" ↔ "Play", `aria-pressed` toggles `false` ↔ `true`, `aria-live` flips `off` ↔ `polite`. 9 new vitest cases.
- **Post-review fixes from PR #327.** Tracked in [`aria-review-fixes.md`](./aria-review-fixes.md). (1) `BsRovingFocus` and `bsCombobox` keyboard handlers now early-return on `Alt`/`Ctrl`/`Meta` so browser/OS chords (Alt+Arrow=back, Ctrl+Home=top of doc, Cmd+Arrow=word jump) reach the platform. Combobox guard is scoped to non-Tab keys since Shift+Tab is a legit chord. (2) `BsOverlayFocus.focusFirstTabbable` swapped its hand-rolled selector for a CDK-`InteractivityChecker`-driven tree walk, so disabled fieldsets / inert ancestors / hidden visibility match CDK's canonical definition. (3) New `BsOverlayStackService` in `@mintplayer/ng-bootstrap/a11y` — a LIFO registry of opaque symbol tokens. `popover`, `tooltip`, `priority-nav`, `dropdown-menu`, and `modal` now push on open / release on close and gate their global Escape handler on `isTop(myToken)`. Closes the popover-under-modal "Escape closes both" bug library-wide; tooltip-under-popover and other nestings get the same fix for free. Tooltip remains transient (still hides on hover/focus loss) but no longer races with higher overlays for Escape.
