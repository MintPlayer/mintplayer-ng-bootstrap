...
- 🟡 **prefers-reduced-motion** on remaining components — partially shipped. ✓ Splitter / dock / scheduler in `332d2d4f`; ✓ marquee already had it; ✓ tile-manager from `ca7a0c9d`; ✓ carousel auto-advance shipped with the APG bundle 2026-05-11 (suppresses the timer entirely when reduce-motion is on, matching tile-manager / scheduler / dock). ⏳ Still pending: parallax scroll effect, accordion open animation.
