...
- [ ] **FR-4** — All nine item kinds, each as its own Lit element + Angular wrapper: Button, SplitButton, DropdownButton, ToggleButton, CheckBox, ComboBox, ColorPicker, GroupButton (toggle strip), Gallery (+ GalleryItem), and a TemplateItem custom-slot wrapper. *(Lit elements: all nine present. Angular wrappers landed: Button, SplitButton, DropdownButton, ToggleButton, CheckBox, ComboBox (with `ControlValueAccessor`). Still missing: Angular wrappers for ColorPicker, GroupButton, Gallery; the `mp-ribbon-gallery-item` and `mp-ribbon-template-item` Lit elements.)*
