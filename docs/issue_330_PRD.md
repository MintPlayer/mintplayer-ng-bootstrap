...
**Issue**: #330
**Title**: Ribbon
**Status**: In Progress — Milestones 1/2/3/6/7 landed (modulo small TODOs), Milestone 8 partially. Milestones 4 (KeyTips) and 5 (Simplified layout) untouched. Core ribbon + all ten item kinds + menus + dark mode (all four versions) + contextual tabs + Quick Access Toolbar + live-announcer wiring + full keyboard model (Ctrl+F1, dblclick, Ctrl+arrows, APG roving-tabindex, popup auto-focus, shared Esc-stack across overlays) + group toolbar ARIA + RTL (logical CSS + arrow-direction flip) + demo keymap legend + code samples have all shipped. Remaining: KeyTips (FR-12), Simplified layout proper (FR-8/FR-39), prefers-reduced-motion (FR-18), `[reduceOrder]`/`[idealSizes]` author API + intermediate sizes (FR-6), slot-based icons (FR-16), `data-size` attribute rename (FR-5), `[priority]` hint + `tellMeSlot` (FR-23/24), tests + Firefox smoke (FR-21/22).
**Created**: 2026-05-12
**Last Updated**: 2026-05-12 (post-FR-7/19/20 closure)
...
- **Demo Code Samples** (FR-20): six copy-paste-ready snippets below the keymap legend covering minimal ribbon, split-button "last-used" pattern, value-bearing items, contextual tab set, QAT, and version + dark mode + app-accent.
- Demo route `/advanced/ribbon` with full Home/Insert/Design/Layout content + Picture Tools contextual set + Quick Access Toolbar (Save / Undo / Redo).
...
- KeyTips (FR-12).
- Esc-stack threading through `BsOverlayStackService` for nested overlays (FR-14 outstanding refinement — single-overlay Esc works today).
...
- [ ] **FR-8** — Classic and Simplified layouts, switchable via `[layout]` attribute. State (selected tab, QAT pins, minimized flag) preserved across switches. *(`[layout]` attribute is plumbed onto `mp-ribbon` but currently has no visual effect — render path is identical for both modes. FR-39 spells out the Simplified-layout spec in detail, including the end-of-tab shared overflow chevron that's the Office-faithful narrow-width / mobile answer.)*
...
- [x] **FR-20** — Demo page at `/advanced/ribbon` covering: full Home tab, Insert tab, Picture Tools contextual tab, QAT, layout toggle, minimize toggle, KeyTips legend, code-block snippets. *(Landed for everything except the KeyTips legend — that one's parked behind FR-12. Route + demo component exist; Home + Insert + Design + Layout tabs populated; Picture Tools contextual set with Format + Effects tabs (toggled via "Select picture" button); QAT (Save / Undo / Redo) above the ribbon; Version / App-accent / Color-scheme / Touch-mode / Direction pickers; layout + minimize toggles; comprehensive **Keyboard shortcuts** legend section below the ribbon documenting tab strip, group, cross-group, dropdown/menu, ribbon-level, and QAT keymaps + the screen-reader live-region behaviour; **Code samples** section with six copy-paste-ready snippets covering minimal ribbon, split-button "last-used" pattern, value-bearing items with `[(ngModel)]`, contextual tab set, QAT, and version + dark mode + app-accent.)*
...
- [ ] **FR-39** — Promote **Simplified layout (FR-8)** from "Should Have / partial" to the **mobile fallback story**. Currently `[layout]="simplified"` is accepted but renders identically to Classic. Spec for the proper implementation:
   ...
   - Layout choice is **consumer-controlled**, NOT auto-flipped on narrow viewport. Decoupling viewport size from layout *mode* — Simplified is a visual aesthetic, not a responsive fallback. Auto-flipping would surprise consumers using Classic who shrink the window.
   - Tab strip + popup overlay touch-target rules from FR-37 apply on top.
...
### Milestone 1: Skeleton & sizing engine — *Mostly Landed*
- [x] Entry point + path alias + Nx wiring
- [x] `mp-ribbon` + `mp-ribbon-tab` + `mp-ribbon-group` + `mp-ribbon-button`
- [ ] `ReduceOrderController` (ResizeObserver-driven) *(MVP inlined on `mp-ribbon` + `mp-ribbon-group`; not yet extracted into a reactive controller, no intermediate `medium`/`small` steps, no author API for `[reduceOrder]`/`[idealSizes]`/`[priority]`)*
...
### Milestone 3: Accessibility & keyboard parity — *Mostly Landed*
...
- [ ] `prefers-reduced-motion` plumbing *(FR-18 — not yet)*
