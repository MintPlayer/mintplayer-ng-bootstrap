...
- [ ] **FR-10** — Contextual tab sets via `mp-ribbon-contextual-tab-set`; coloured header band (themed via `--bs-ribbon-contextual-color`); show/hide announced via live region. *(Visual portion v2 landed: consecutive contextual tabs from the same set are wrapped in a `.ribbon-contextual-group` (flex-column) inside `.ribbon-tablist`; the band is the first child, sized to span EXACTLY the tabs below it (no margin, sharp rectangle, flush against the tab strip — matches Office 2013+). Wrapper sets `--bs-ribbon-contextual-color` on itself so the band background and the contextual-tabs' top-border inherit via cascade. Band text colour is computed luminance-driven: if the background's relative luminance ≥ 0.6 the text is `#262626`, else `#FFFFFF` (Office's "dark-on-pastel / white-on-saturated" rule, automatic for any consumer-supplied colour). Active contextual tab keeps the normal `--bs-ribbon-tab-active-color` for text — the 3px coloured top-border is the only contextual indicator (Office's readability trick). Default colour `#F0AF84` (Office 2016 Picture Tools warm-amber estimate; consumer can override). Set fires `contextual-visibility-change` on `hidden` toggle so the ribbon re-processes the slot. If the active tab vanishes (its set just hid), the ribbon falls back to the first visible tab. **Missing: live-region announcement** ("Picture Tools, contextual, now available") on show/hide — depends on FR-17 wiring.)*
