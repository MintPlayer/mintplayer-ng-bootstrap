...
**Issue**: #330
**Title**: Ribbon
**Status**: In Progress — Milestone 7/8. Core ribbon + all ten item kinds + menus + dark mode (all four versions) + contextual tabs (visual) + DOM-as-source-of-truth tabs + mobile/touch accessibility + Quick Access Toolbar + live-announcer wiring + Ctrl+F1/double-click/Ctrl+arrow keyboard shortcuts have all shipped. Remaining: Simplified layout proper (FR-8/FR-39), KeyTips (FR-12), full roving-focus inside groups (FR-14 finishing touches), prefers-reduced-motion (FR-18), tests + Firefox smoke (FR-21/22).
**Created**: 2026-05-12
...
- **Keyboard model expansion** (FR-9/14): `Ctrl+F1` toggles minimize; double-click tab toggles minimize; `Ctrl+←/→` jumps focus between groups in the active tab; `mp-ribbon` dispatches `minimize-toggle` event on user-driven toggles so the Angular wrapper's `model<>` two-way binding syncs the consumer's signal.
...
- KeyTips (FR-12).
- Roving-tabindex inside group toolbars + Esc-stack threading through `BsOverlayStackService` for nested overlays (FR-14 outstanding refinements).
...
- [x] **FR-14** — Keyboard model: arrows in tab strip, Tab/Shift+Tab inside toolbars, Ctrl+←/→ jumps group-to-group, Down opens dropdowns, Space/Enter activates, Esc unwinds. *(Landed: arrow keys + Home/End + Enter/Space in tab strip (FR-2); Ctrl+F1 toggles minimized + double-click tab toggles minimized (FR-9); Ctrl+ArrowLeft/Right jumps focus group-to-group inside the active tab (skipped if focus is on a tab — the tablist handler owns those arrows); Down/Alt+Down opens dropdown menus on split-button / dropdown-button triggers (already shipped in FR-28); Esc closes the topmost open overlay via `OverlayController`'s document-level listener. Native Tab / Shift+Tab moves between focusable items inside a group since each item is a real `<button>`. **Outstanding refinement**: full roving-tabindex inside group toolbars (APG pattern — only one item per toolbar is tabbable, arrow keys navigate within); Esc-stack-aware unwinding via `BsOverlayStackService` (Angular-only service; not yet threaded into the Lit `OverlayController` — single-overlay Esc works correctly today).)*
