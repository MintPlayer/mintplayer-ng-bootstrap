...
**B. Sort-by section in the query-builder.** Infragistics does *not* have this, but the user explicitly wants it. Currently the datatable owns sort state (column-header clicks); there's no way to see / edit the active sort *outside* the datatable's column headers. A sort-by builder in the query-builder UI surfaces it and makes multi-priority sort discoverable. Sort flows two-ways: datatable header click writes to `settings.sortColumns`; the query-builder sort builder reads back from there and edits both. (Stop-gap in PR #340: the `id` column was set `[bsDatatableColumnSortable]="false"` because the backend's `ApplySort` only switch-matches `total` / `orderDate` / `status`. `customerId` / `tags` clicks silently fall back too — to be cleaned up in Phase 2 via FR-55.)
