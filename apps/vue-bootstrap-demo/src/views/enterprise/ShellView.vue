...
const SNIPPET_BASIC = `<script setup lang="ts">
...
const SNIPPET_HEIGHT = `/* The shell becomes its own scroll region (top bar + sidebar stay put,
   ...
   class/style on <BsShell> fall through to the underlying element. */
.app-shell {
  height: 100dvh;
}`;

const SNIPPET_STATE = `<script setup lang="ts">
import { ref } from 'vue';
import { BsShell } from '@mintplayer/vue-bootstrap/shell';
...
const state = ref<'auto' | 'show' | 'hide'>('auto');
function onStatechange(detail: ShellStateChangeEventDetail) {
  state.value = detail.open ? 'show' : 'hide';
}
<\/script>

<template>
  <BsShell breakpoint="md" :state="state" @statechange="onStatechange">
...
<BsShell external-toggle breakpoint="md">
  <nav slot="sidebar">…</nav>
  <main>…</main>
</BsShell>`;
</script>
...
<template>
  <div class="demo-page">
    ...
    <section>
      <h2>Basic usage</h2>
      <p>
        A full-width top bar (built-in hamburger + <code>topbar</code> slot) sits
        above the sidebar/content row. <code>breakpoint</code> sets the viewport
        width at/above which the sidebar is expanded; <code>size</code> is the
        expanded width.
      </p>
      <BsCodeSnippet :code="SNIPPET_BASIC" language="html" />
    </section>

    <section>
      <h2>Full-height layout</h2>
      <p>
        The content is the scroll region only when the host has a bounded height.
        Give <code>BsShell</code> a viewport height — anything you pass via
        <code>class</code> or <code>style</code> falls through to the element.
      </p>
    ...
    <section>
      ...
      <p>
        Leave <code>state</code> as <code>"auto"</code> for responsive behaviour,
        or drive it yourself and react to <code>@statechange</code> (its
        <code>detail.open</code> is the resolved visual state).
      </p>
      <BsCodeSnippet :code="SNIPPET_STATE" language="html" />
    </section>

    <section>
      <h2>External toggle</h2>
      <p>
        Set <code>external-toggle</code> to hide the built-in hamburger and toggle
        the shell from your own button.
      </p>
      <BsCodeSnippet :code="SNIPPET_EXTERNAL" language="html" />
    </section>
    ...
    <section>
      ...
      <table class="table table-sm">
        ...
        <tbody>
          ...
          <tr>
            <td><code>external-toggle</code></td>
