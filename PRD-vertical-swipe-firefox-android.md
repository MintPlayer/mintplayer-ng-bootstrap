...
**Status:** Original fix shipped in [PR #291](https://github.com/MintPlayer/mintplayer-ng-bootstrap/pull/291), validated on real Firefox Android device 2026-04-30. §11 follow-up (directive consolidation) still open.
**Author:** Pieterjan
**Date:** 2026-04-30
...
1. Vertical and horizontal carousels render and behave identically to before, both visually and in terms of swipe response. Pixel-diff demo screenshots before/after.
2. Original §7 acceptance criteria (Firefox Android PTR, regression tests) still pass.
3. Generated DOM has one fewer `<div>` between `.carousel` and the slides.
4. `nx test mintplayer-ng-swiper` passes.
5. `nx build mintplayer-ng-swiper` and `nx build mintplayer-ng-bootstrap` pass.
