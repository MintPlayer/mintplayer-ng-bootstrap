...
- **`@mintplayer/ng-bootstrap/navbar`** — `BsNavbarComponent` no longer emits the deprecated `.navbar-light` / `.navbar-dark` classes on its rendered `<nav>`. Bootstrap 5.3 deprecated both; the component now writes `[data-bs-theme="light|dark"]` directly on the `<nav>` per Bootstrap's recommended pattern, and emits a `bg-{color}` utility class to set the background. `[color]` also widened from `Color | null` to `Color | string | null` — string values (e.g. `[color]="'body-tertiary'"`) emit `bg-{value}` and let the page theme cascade through (no `data-bs-theme` override). Migration: only affects consumers with custom CSS keyed off `.navbar-dark` / `.navbar-light` (no such usage in this repo). See `docs/issue_324_navbar_modernize_PRD.md` for the full mapping table.

### Added

- `@mintplayer/ng-bootstrap/theming`: new package. `BsThemeService` is a signal-first service that owns the user's Bootstrap color-mode choice and writes the resolved value to `<html data-bs-theme>`. Methods: `setMode('auto' | 'light' | 'dark' | string)`. Signals: `mode` (authored), `effectiveMode` (resolved — `auto` resolves to `light` / `dark` via `matchMedia('(prefers-color-scheme: dark)')` and live-updates when the OS preference changes). State persists to `localStorage` under `BS_THEME_STORAGE_KEY` (`'bs-theme-mode'`). The string-typed mode admits custom variants — ship your own `[data-bs-theme="sepia"] { … }` block and call `setMode('sepia')`. SSR-safe (no `localStorage` / `matchMedia` / DOM access on the server). Pair with a tiny inline pre-boot `<script>` in `<head>` to apply the persisted mode before any CSS evaluates and prevent a light-mode flash on reload — see `/additional-samples/theming` in the demo for the full recipe (build-time SCSS overrides, runtime `--bs-*` mutation, mode switching, per-component variable reference, custom variants, SSR integration).
