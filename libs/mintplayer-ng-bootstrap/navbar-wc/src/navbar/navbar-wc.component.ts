...
/**
 * `<bs-navbar-wc>` — Angular wrapper around the `<mp-navbar>` web component.
 *
 ...
 * slots:
 *  - mark the brand with `bs-navbar-wc-brand` (→ `slot="brand"`);
...
export class BsNavbarWc {
  ...
  protected onExpandedchange(event: Event) {
    ...
    // consumer's `<bs-navbar-wc>` host, where their `(expandedchange)` binding
