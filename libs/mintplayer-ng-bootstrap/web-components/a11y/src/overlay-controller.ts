...
/**
 * Reusable popup-overlay primitive for Lit elements.
 *
 * Owns: open/close state, viewport-clamped horizontal positioning, vertical
 * flip when the panel would overflow the viewport bottom, Escape-to-close,
 * outside-mousedown-to-close, focus return to trigger.
 *
 * Re-renders the host via `requestUpdate()` on state changes so the host
 * template can bind `aria-expanded` to `overlay.isOpen` and reveal/hide the
 * panel via a CSS rule keyed on the host's `data-menu-open` attribute.
 *
 * A library-wide LIFO stack ensures Esc and outside-mousedown only fire on the
 * topmost overlay — nested popups (popup-inside-popup) unwind one at a time.
 */
export class OverlayController implements ReactiveController {
   ...
   * `pushFrame()` with a `releaseFrame(token)` to keep the stack consistent.
   */
