...
@Directive({
  selector: '[bsHasProperty]',
})
export class BsHasPropertyDirective {
  private _defaultViews!: SwitchView[];
  private _defaultUsed = false;
  private _caseCount = 0;
  private _lastCaseCheckIndex = 0;
  private _lastCasesMatched = false;
  private _hasProperty: any;

  @Input()
  public set bsHasProperty(newValue: any) {
    this._hasProperty = newValue;
  ...
  public _matchCase<T>(type: AbstractType<T>): T | undefined {
    const matched =
      this._hasProperty instanceof type
        ? this._hasProperty
        : undefined;
