import { Component, Input, ContentChildren, QueryList, ElementRef, HostListener, HostBinding, ViewChildren, signal, computed, effect, untracked } from '@angular/core';
...
export class BsSplitterComponent {
  ...
  //#region Orientation
  orientationSignal = signal<Direction>('horizontal');
  @Input() set orientation(val: Direction) {
    this.orientationSignal.set(val);
  }
  //#endregion
  ...
  @ContentChildren(BsSplitPanelComponent) set panelsQuery(value: QueryList<BsSplitPanelComponent>) {
